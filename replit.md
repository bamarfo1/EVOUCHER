...
- **Purchase Options**: Supports purchases with only a phone number (SMS delivery) or both email and phone. Users can also select a **quantity** (1 up to available stock, capped at 10) to purchase multiple vouchers in a single transaction. The system handles cases without email by generating a placeholder email (`{phone}@noemail.alltekse.com`) for Paystack and skipping email notifications.
...
- **Vendor System**: Authorised resellers can sign up at `/vendor/signup` with phone, password, MoMo number/name, and contact number. After login, vendors access a dashboard at `/vendor/dashboard` to: view their store links (`/v/:slug` and optionally a short subdomain URL like `tekse.online/:subdomain`), set custom prices per card type (minimum = base price; profit = vendor price − base price), view their sales stats, edit their store name inline, set a custom subdomain URL, and **request withdrawals** (see below). Their public page works at both `/v/:slug` and `/:subdomain` and shows their store name, a WhatsApp Support button, a Call button, and the purchase flow using their prices. The same voucher retrieve page is shared.
...
- **Frontend Pages**:
    ...
    - `/v/:slug` — Public vendor store page (original format)
    - `/:subdomain` — Public vendor store page (short custom URL format, e.g. `/brightshop`)
